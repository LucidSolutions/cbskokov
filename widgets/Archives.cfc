...
component extends="contentbox.models.ui.BaseWidget" singleton{
	...
	*/
	...
	private function buildList(archives,showPostCount){
		...
		saveContent variable="rString"{
			writeOutput('<ul id="archives" class="blog-list">');
			// iterate and create
			for(var x=1; x lte arrayLen( arguments.archives ); x++){
				var thisDate = arguments.archives[ x ]["year"] & "-" & arguments.archives[ x ]["month"] & "-1";
				writeOutput('<li><img src="#cb.themeRoot()#/includes/images/arrow.png" alt=""><p><a href="#cb.linkArchive(year=arguments.archives[ x ]['year'],month=arguments.archives[ x ]['month'])#">#dateFormat(thisDate,"mmmm yyyy" )#');
				if( arguments.showPostCount ){ writeOutput( " (#arguments.archives[ x ]['count']#)" ); }
				writeOutput('</a></p><div class="clear"></div></li>');
			}
			// close ul
			writeOutput( "<div class='clear'></div></ul>" );
		}
		return rString;
	}
